...
local props = {
	...
	-- meth
	["meth_bag"] = { model = "hei_prop_pill_bag_01", hash = joaat("hei_prop_pill_bag_01"), tier = 3 },
	["coke_figure"] = { model = "hei_prop_pill_bag_01", hash = joaat("hei_prop_pill_bag_01"), tier = 3 },
	...
	-- I us=e these for my house robbery when they steal the objects --
...
local function DeleteWeapon(item)
	...
	while GetSelectedPedWeapon(ped) ~= hash do
		Wait(1000)
		wait = wait + 1
		if wait >= 45 then return end -- If they figure out a way to spam then this we just return the function
	end
