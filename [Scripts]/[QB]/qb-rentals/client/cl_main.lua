...
RegisterNetEvent('qb-rental:client:openMenu', function(data)
    ...
    elseif menu == "vrental2" then
        for k=1, #Config.Vehicles.vrental2 do
            local veh = QBCore.Shared.Vehicles[Config.Vehicles.vrental2[k].model]
            local name = veh and ('%s'):format(veh.name) or Config.Vehicles.vrental2[k].model:sub(1,1):upper()..Config.Vehicles.vrental2[k].model:sub(2)
            vehMenu[#vehMenu+1] = {
                id = k+1,
                header = name,
                txt = ("£%s"):format(comma_value(Config.Vehicles.vrental2[k].money)),
                params = {
                    event = "qb-rental:client:spawncar",
                    args = {
                        model = Config.Vehicles.vrental2[k].model,
                        money = Config.Vehicles.vrental2[k].money,
                    }
                }
            }
            exports['qb-menu']:openMenu(vehMenu)
...
local CreateNPC = function()
    ...
    -- VRentals 2
    created_ped = CreatePed(5, Config.Locations.vrental2.pedhash, Config.Locations.vrental2.coords.x, Config.Locations.vrental2.coords.y, Config.Locations.vrental2.coords.z, Config.Locations.vrental2.coords.w, false, true)
    FreezeEntityPosition(created_ped, true)
    SetEntityInvincible(created_ped, true)
    SetBlockingOfNonTemporaryEvents(created_ped, true)
    TaskStartScenarioInPlace(created_ped, 'WORLD_HUMAN_CLIPBOARD', 0, true)
...
local SpawnNPC = function()
    CreateThread(function()
        ...
        -- Vrental2
        RequestModel(Config.Locations.vrental2.pedhash)
        while not HasModelLoaded(Config.Locations.vrental2.pedhash) do
            Wait(1)
        end
        -- Boat Rentals
...
RegisterNetEvent('qb-rental:client:spawncar', function(data)
    ...
    elseif menu == "vrental2" then
        if IsAnyVehicleNearPoint(Config.Locations.vrental2.spawnpoint.x, Config.Locations.vrental2.spawnpoint.y, Config.Locations.vrental2.spawnpoint.z, 2.0) then 
            QBCore.Functions.Notify(label, "error", 4500)
            return
        end 
    elseif menu == "boat" then
    ...
    QBCore.Functions.TriggerCallback("qb-rental:server:CashCheck",function(money)
        if money then
            ...
            elseif menu == "vrental2" then
                QBCore.Functions.SpawnVehicle(model, function(vehicle)
                    SetEntityHeading(vehicle, Config.Locations.vrental2.spawnpoint.w)
                    TaskWarpPedIntoVehicle(player, vehicle, -1)
                    TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(vehicle))
                    SetVehicleEngineOn(vehicle, true, true)
                    SetVehicleDirtLevel(vehicle, 0.0)
                    exports[Config.FuelExport]:SetFuel(vehicle, 100)
                    SpawnVehicle = true
                end, Config.Locations.vrental2.spawnpoint, true)
            elseif menu == "aircraft" then
                QBCore.Functions.SpawnVehicle(model, function(vehicle)
