...
RegisterNetEvent('jim-mechanic:client:applyBrakes', function(level) local Ped = PlayerPedId()
	...
	if DoesEntityExist(vehicle) then
		...
		if level ~= true then
			...
			if GetNumVehicleMods(vehicle, 12) == 0 or level > GetNumVehicleMods(vehicle, 12) then triggerNotify(nil, Loc[Config.Lan]["common"].cant, "error") return end
...
RegisterNetEvent('jim-mechanic:client:applySuspension', function(level) local Ped = PlayerPedId() emptyHands(Ped)
	...
	if DoesEntityExist(vehicle) then
		...
		if level ~= true then
			...
			if (GetNumVehicleMods(vehicle, 15) == 0 and (level+1) > GetNumVehicleMods(vehicle, 15)) then triggerNotify(nil, Loc[Config.Lan]["common"].cant, "error") return end
...
RegisterNetEvent('jim-mechanic:client:applyTransmission', function(level) local Ped = PlayerPedId() emptyHands(Ped)
	...
	if DoesEntityExist(vehicle) then
		...
		if level ~= true then
			...
			if (GetNumVehicleMods(vehicle, 13) == 0 or (level+1) > GetNumVehicleMods(vehicle, 13)) then triggerNotify(nil, Loc[Config.Lan]["common"].cant, "error") return end
...
RegisterNetEvent('jim-mechanic:client:applyEngine', function(level) local Ped = PlayerPedId() emptyHands(Ped)
	...
	if DoesEntityExist(vehicle) then
		...
		if level ~= true then
			...
			if (GetNumVehicleMods(vehicle, 11) == 0 or (level+1) > GetNumVehicleMods(vehicle, 11)) then triggerNotify(nil, Loc[Config.Lan]["common"].cant, "error") return end
