...
AddEventHandler("okokChat:ServerMessage", function(background, color, icon, title, playername, message, target, image)
	...
	if image == nil or image:gsub("%s+", "") == "" then
		TriggerClientEvent('chat:addMessage', target, {
			template = '<div class="chat-message" style="background: {0};"><i class="{2}" style="color: #fff; background-color: {1}; padding: 5px; border-radius: 5px;"></i> <b><span style="color: {1}">[{3}]{4}</span>&nbsp;<span class="time">{6}</span></b><div class="message">'..message..'</div></div>',
			args = { background, color, icon, title, playername, message, time }
		})
	else
		TriggerClientEvent('chat:addMessage', target, {
			template = '<div class="chat-message" style="background: {0};"><i class="{2}" style="color: #fff; background-color: {1}; padding: 5px; border-radius: 5px;"></i> <b><span style="color: {1}">[{3}]{4}</span>&nbsp;<span class="time">{6}</span></b><div class="message">'..message..'<br><img src="'..image..'" width="100%"></img></div></div>',
			args = { background, color, icon, title, playername, message, time }
		})
	end
end)
...
if Config.EnableMe then
	RegisterCommand(Config.MeCommand, function(source, args, rawCommand)
		...
		if message:gsub("%s+", "") ~= "" then
			TriggerClientEvent('chat:3D', -1, message, source, "me")
		end
	end)
end
...
if Config.EnableDo then
	RegisterCommand(Config.DoCommand, function(source, args, rawCommand)
		...
		if message:gsub("%s+", "") ~= "" then
			TriggerClientEvent('chat:3D', -1, message, source, "do")
		end
	end)
end
...
if Config.EnableTry then
	RegisterCommand(Config.TryCommand, function(source, args, rawCommand)
		...
		local message = rawCommand:sub(length + 1)
		local doTry = math.random(0, 1)

		if doTry == 0 then return end
		if message:gsub("%s+", "") ~= "" then
			TriggerClientEvent('chat:3D', -1, message, source, "try")
		end
	end)
end
...
Citizen.CreateThread(function()
    while Config.EnableAutoMessage do
		...
		TriggerClientEvent('chat:addMessage', -1, {
			template = '<div class="chat-message server-msg"><i class="'..Config.AnnouncementIcon..'" style="vertical-align: middle;"></i> <span style="vertical-align: middle;"><b><span style="color: #cc3d3d;">['..Config.AnnouncementMessageTitle..']</span>&nbsp;<span class="time">{1}</span></b></span><div class="message">{0}</div></div>',
		...
		if message:gsub("%s+", "") ~= "" then
			if isAdmin(xPlayer) then
				showToEveryoneNotHidden(function(players)
					TriggerClientEvent('chat:addMessage', players, {
						template = '<div class="chat-message staff"><i class="'..Config.StaffIcon..'" style="vertical-align: middle;"></i> <span style="vertical-align: middle;"><b><span style="color: #1ebc62;">['..Config.StaffMessageTitle..'] {0}</span>&nbsp;<span class="time">{2}</span></b></span><div class="message">{1}</div></div>',
...
function showToEveryoneNotHidden(players)
	for k,v in ipairs(ESX.GetPlayers()) do
		local xPlayer = ESX.GetPlayerFromId(v)
		if Config.ShowStaffMessageWhenHidden then
			players(v)
		else
			if chatsHidden[xPlayer.source] == nil then
				players(v)
			end
		end
	end
...
function showToClosePlayers(xPlayer, admins)
	...
	for k,v in ipairs(ESX.GetPlayers()) do
		...
		if not Config.ShowIDOnMessage and chatsHidden[xTarget.source] == nil then
			...
			if #(vector3(playerCoords.x, playerCoords.y, playerCoords.z) - vector3(targetCoords.x, targetCoords.y, targetCoords.z)) < Config.OOCDistance then
				admins(v)
			end
